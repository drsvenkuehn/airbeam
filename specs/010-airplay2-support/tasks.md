...
- [ ] T006 [P] Extend `src/discovery/AirPlayReceiver.h`: add `supportsAirPlay2` (bool), `hapDevicePublicKey` (std::string, base64 Ed25519 from TXT `pk`), `pairingState` (PairingState enum), `airPlay2Port` (uint16_t); add `PairingState` enum with values `NotApplicable / Unpaired / Pairing / Paired / Error`
- [ ] T007 [P] Update `src/discovery/TxtRecord.h/.cpp`: parse `pk` TXT field into `hapDevicePublicKey`; parse `vv` field; set `supportsAirPlay2 = !pk.empty()`; set `isAirPlay2Only = supportsAirPlay2 && et_has_no_rsa()`; set `airPlay2Port` from TXT-advertised port (fallback 7000)
- [ ] T007a Update `src/discovery/MdnsDiscovery.h/.cpp`: register a second `DNSServiceBrowse()` call for `_airplay._tcp` service type alongside the existing `_raop._tcp` browse; wire its callback into the existing discovery pipeline so that AirPlay 2-only receivers (HomePod) that do NOT advertise `_raop._tcp` enter `ReceiverList`; deduplicate by `stableId` if a receiver advertises both service types; **depends on T006** (requires updated `AirPlayReceiver` fields — no `[P]` marker; start only after T006 is complete)
- [ ] T008 [P] Update `src/discovery/ReceiverList.h/.cpp`: merge logic correctly handles receivers with both `supportsAirPlay2` and `isAirPlay1Compatible` set; stale-entry removal (60 s) applies equally to AP2 receivers
- [ ] T009 [P] Add `WM_AP2_PAIRING_REQUIRED`, `WM_AP2_PAIRING_STALE`, `WM_AP2_CONNECTED`, `WM_AP2_FAILED`, `WM_AP2_SPEAKER_DROPPED` constants to `src/core/Messages.h` following existing WM_RAOP_* naming convention
