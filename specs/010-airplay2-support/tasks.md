...
- [ ] T007a [P] Update `src/discovery/MdnsDiscovery.h/.cpp`: register a second `DNSServiceBrowse()` call for `_airplay._tcp` service type alongside the existing `_raop._tcp` browse; wire its callback into the existing discovery pipeline so that AirPlay 2-only receivers (HomePod) that do NOT advertise `_raop._tcp` enter `ReceiverList`; deduplicate by `stableId` if a receiver advertises both service types; **depends on T006** (requires updated `AirPlayReceiver` fields)
...
- [ ] T029 [P] [US1] Add `tests/integration/test_airplay2_session.cpp`: full AP2 session setup against a shairport-sync instance compiled with AP2 support (Docker/WSL2); verify SETUP succeeds, audio flows, SET_PARAMETER (volume) accepted, TEARDOWN clean; assert wall-clock time from `Init()` call to `WM_AP2_CONNECTED` receipt ≤3 s (SC-003 gate); inject a time-marked ALAC frame and measure arrival timestamp at receiver — assert round-trip ≤2 s (SC-004 gate); validate `WM_AP2_PAIRING_STALE` is posted (not `WM_AP2_FAILED`) on simulated HAP auth rejection; validate `AP2_ERROR_PORT_UNREACHABLE` is posted when control port is firewalled
